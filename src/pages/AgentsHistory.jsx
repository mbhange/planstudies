...
const AgentHistory = () => {
  ...
  const searchFilteredAgents = filteredAgents.filter(
    (agent) =>
  ...
  const totalPages = Math.ceil(searchFilteredAgents.length / agentsPerPage);
  const indexOfLastAgent = currentPage * agentsPerPage;
  const indexOfFirstAgent = indexOfLastAgent - agentsPerPage;
  const paginatedAgents = searchFilteredAgents.slice(
    indexOfFirstAgent,
    indexOfLastAgent
  );
  ...
  }, [agents, filterType, selectedYear, selectedMonth, selectedWeek, selectedDay, customRange]);
  ...
  const exportToCSV = () => {
    const csv = Papa.unparse(searchFilteredAgents);
    const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" });
    saveAs(blob, `agents_${new Date().toISOString().split("T")[0]}.csv`);
  };
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                  <h1 className="text-2xl md:text-4xl font-bold flex items-center gap-3">
                    <FaUserTie className="text-3xl md:text-4xl" />
                    Agent Enrollment History
                  </h1>
                  <p className="text-blue-100 mt-2 text-sm md:text-base">
                    Track and analyze agent enrollment trends over time
                  </p>
                </div>
          ...
          <div className="bg-white border-gray-200 rounded-2xl shadow-lg overflow-hidden">
            <div className="p-6 border-b border-gray-200">
              <h2 className="text-xl font-semibold text-gray-800">
                Agent Records ({searchFilteredAgents.length} results)
              </h2>
            </div>
