...
const odooRpcClient = new OdooClient({
    url: `${ODOO_URL}:${ODOO_PORT}`,
    db: ODOO_DB,
...
try {
  ...
  console.log("Connection to Odoo sever is Successful")
} catch (error) {
...
app.post("/api/login", (req, res) => {
        ...
        authenticateUser.connect(function (err) {
            if (err) {
            ...
            var inParams = [];
            inParams.push([['login', '=', email]]);
            inParams.push(['login', 'id', 'name']); //fields
            var params = [];
            ...
            odoo.execute_kw('res.users', 'search_read', params, function (err, value) {
                ...
                console.log('Result: ', value);
                 return res.status(200).json({
                   ...
                   user: { id: value[0].id, email: value[0].login, role: 'student' } // ✅ Return user data
               });
            });
        });
    });
...
app.post("/api/register", async (req, res) => {
  ...
  try {
        ...
        odoo.connect(function (err) {
            ...
            var inParams = [];
            inParams.push({'name': fullName,'login':email, 'password':password})
            var params = [];
            params.push(inParams);
            odoo.execute_kw('res.users', 'create', params, function (err, value) {
                if (err) {
                return res.status(400).send(err.faultString);
                }
                console.log('Result: ', value);
...
//    }
//  });
    res.json({ fullname: "yashveer Singh" });
});
...
//    const countResponse = await axios.post('http://flc009:8069/', {
//      jsonrpc: "2.0",
//      method: "call",
//      params: {
//        service: "object",
...
//    });
    ...
    console.log("INSIDE");
        ...
        var params = [];
        params.push(inParams);
        console.log(params,"LIne 500")
        return odoo.execute_kw('flc.course', 'search_count', params, function (err, value) {
...
//    console.log(odoo,"odoooooo")
//    console.log(countResponse,"ERRRRRRRRRRRORRRRRRRR");
...
//    const totalPages = Math.ceil(totalCourses / resultsPerPage);
    ...
    if (fetchAll) {
      ...
      for (let i = 0; i < totalCourses; i += batchSize) {
        const batchResponse = await axios.post('http://flc009:8069/', {
          jsonrpc: "2.0",
          method: "call",
          params: {
            service: "object",
...
//      return res.status(500).json({ error: "Database error. Check server logs." });
//    }
    res.json({sender_email: 'sender@mail.com', receiver_email: 'receiver@mail.com', message: 'SUCESSSSSSSSSSSSS', file_url: '/home/downloads'})
//  });
