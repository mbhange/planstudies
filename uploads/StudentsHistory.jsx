...
const StudentHistory = () => {
    ...
    const uniqueYears = [...new Set(students.map((s) => new Date(s.created_at).getFullYear()))];

    const filterData = () => {
        let filteredData = students;
        if (filterType === "year" && selectedYear) {
            filteredData = students.filter((s) => new Date(s.created_at).getFullYear() === parseInt(selectedYear));
            ...
            filteredData = students.filter((s) => new Date(s.created_at).toISOString().split("T")[0] === selectedDay);
    ...
    return (
            ...
            <div className={darkMode ? "bg-gray-900 text-white p-6" : "p-6"}>
                ...
                {filterType === "year" && (
                    <select onChange={(e) => setSelectedYear(e.target.value)} className={`p-2 border rounded ${darkMode ? "bg-gray-700 text-white border-gray-500" : "bg-white text-black border-gray-300"}`}>
                        ...
                        {[...new Set(students.map((s) => new Date(s.created_at).getFullYear()))].map((year) => (
                ...
                <input type="date" value={customRange.from} onChange={(e) => setCustomRange({ ...customRange, from: e.target.value })} className={`p-2 border rounded ${darkMode ? "bg-gray-700 text-white border-gray-500" : "bg-white text-black border-gray-300"}`} />
                <input type="date" value={customRange.to} onChange={(e) => setCustomRange({ ...customRange, to: e.target.value })} className={`p-2 border rounded ${darkMode ? "bg-gray-700 text-white border-gray-500" : "bg-white text-black border-gray-300"}`} />
                <button onClick={filterByCustomDate} className="p-2 bg-green-600 text-white rounded">Apply</button>
            </div> */}
                    ...
                    <table className="min-w-full border-collapse border mt-4">
                        {/* <thead>
                            <tr className={`${darkMode ? "bg-gray-700 text-white" : "bg-gray-300 text-black"} text-left`}>
                                <th className="border p-3 text-sm md:text-base">Enrollment Date</th>
                                <th className="border p-3 text-sm md:text-base">Student Name</th>
                                <th className="border p-3 text-sm md:text-base">Phone Number</th>
                                <th className="border p-3 text-sm md:text-base">Email</th>
                            </tr>
                        </thead>
                        <tbody>
                            {filteredStudents.map((student) => (
                                <tr key={student.id} className="text-center">
                                    <td className="border p-2 text-xs md:text-sm lg:text-base">{new Date(student.created_at).toLocaleDateString()}</td>
                                    <td className="border p-2 text-xs md:text-sm lg:text-base">{student.fullname}</td>
                                    <td className="p-3 border">
                                        <a href={`tel:${student.phone_number}`} className="text-blue-500 hover:underline">
                                            {student.phone_number}
                                        </a>
                                    </td>
                                    <td className="border p-2 text-xs md:text-sm lg:text-base">{student.email}</td>
                                </tr>
                            ))}
                        </tbody> */}
                        <thead>
                        ...
                        <tbody>
                            {paginatedStudents.length > 0 ? (
                                paginatedStudents.map((student) => (
                                    <tr key={student.id} className="text-center">
                                        <td className="border p-2 text-xs md:text-sm lg:text-base">
                                ...
                                <tr>
                                    <td colSpan="4" className="p-3 border text-center text-gray-500">No students found.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
        ...
        <div className="flex justify-center gap-2 mt-4">
          ...
          <button
            ...
            className={`px-4 py-2 rounded ${
              currentPage === totalPages ? "bg-gray-300 text-gray-600 cursor-not-allowed" : "bg-blue-500 hover:bg-blue-700 text-white"
            }`}
          >
            Next
          </button>
